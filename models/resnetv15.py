...
class Bottleneck(nn.Module):
    ...
    def __init__(self, builder, inplanes, planes, stride=1, downsample=None):
        ...
        # self.gate_residual = GateLayer(planes * self.expansion, planes * self.expansion, [1, -1, 1, 1], group = builder.layer_index)
        self.relu = builder.activation()
        self.downsample = downsample
        self.stride = stride
    ...
    def forward(self, x):
        ...
        #     if not self.skip_layer1:
        #         if self.gate1.weight.sum() == 0:
        #             self.skip_layer1 = True
        #
        #     if not self.skip_layer2:
        #         if self.gate2.weight.sum() == 0:
        #             self.skip_layer2 = True
        #
        #     # if self.skip_layer1 or self.skip_layer2:
        #     #     out = self.relu(residual)
        #     #     return out
        #     if self.skip_layer1 or self.skip_layer2:
        #         ## even if the first conv is zero we still have to use the last BN layer
        #         if 0:
        #             if not self.skip_layer2:
        #                 n_elements = self.gate2.weight.shape[0]
        #                 out = self.bn2(x.detach()[:,:n_elements,:,:]*0.0)
        #                 out = self.relu(out)
        #                 out = self.gate2(out)
        #                 out = self.conv3(out)
        #             else:
        ...
        #             if self.downsample is not None:
        #                 residual = self.downsample(x)
        #                 #in case of downsample layer we need to be prepared
        #                 if residual.shape[1] > out.shape[1]:
        #                     out = residual.detach() * 0.0
        #
        #             out = self.bn3(out)
        #
        #             out += residual
        ...
        # new oct 2019
        # if self.gate1.weight.sum()==0 or self.gate2.weight.sum()==0:
        #     out = out * 0.0
        ...
        out = self.relu(out)
        # if hasattr(self, 'gate_residual'):
        #     out = self.gate_residual(out)
